-- VertexED cloud sync — run once in Supabase SQL Editor
-- Dashboard → SQL → New query → paste this entire file → Run
-- Safe to run multiple times (idempotent).

-- User study artifacts (notes, reviews, papers, planner, notebook)
...
-- Upgrade kind constraint if table existed with older check
...
-- Profile fields used by onboarding v2 (optional)
