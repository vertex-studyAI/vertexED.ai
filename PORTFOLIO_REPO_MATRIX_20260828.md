...
| `vertex-studyAI/vertexED.ai` — rolling repo matrix | PR #549 branch `ops/portfolio-repo-matrix-20260828` | Open, non-draft, mergeable; prior head `1d5bd815c47a91ef093b84b15e4d5d394b454e6e` triggered fresh CI before this second reconciliation edit | Fresh CI required after this matrix update | Verify the new exact head after this commit and keep this file as the canonical connected-repo truth surface | Prior head `942f7f29dcad57f7cbd5435ca7f4def55d1d5f04` passed CI `33210860807` |
| `vertex-studyAI/Text-To-Video` — repository controls | `main` @ `15af0cf315fcf53235c7f4f2f3f90712fc13c5fe` | `main` remains unprotected; required status checks are disabled | PR/CI workflow is not enforced by repository settings; connected GitHub write surface does not expose repository-admin/ruleset mutation | Owner/admin configures branch protection/ruleset and then re-read state | Fresh post-merge branch read; issue #36 tracks exact acceptance gate |
| `vertex-studyAI/Text-To-Video` — local artifact store status | `main` @ `15af0cf315fcf53235c7f4f2f3f90712fc13c5fe` | PR #32 merged through normal PR path; canonical main now contains the reconciled local artifact-store truth boundary | Hosted/production storage is still not established | Keep production storage/hosting claims separate; no further repo gate remains for this status reconciliation | Merge commit `15af0cf315fcf53235c7f4f2f3f90712fc13c5fe`; source PR head CI `31932198400` SUCCESS |
