...
| Darcy T2424-0050 | **GREEN — bounded synthetic mechanism; v2 scientific protocol frozen, NOT RUN** | `LEARNED_OPERATOR_OOD_PROTOCOL_V2.md` is frozen on canonical main via `6fbd9c4ba73a460f5abbe6a6f4c478b6bc50e389`; it preregisters matched reduced/learned baselines, ID/OOD families, metrics, seeds, uncertainty and falsifiers. Execution remains unauthorized until the exact learned implementations/library versions, parameter budgets, optimizer/tuning grid, hardware identity and compute/time cap required by the protocol are committed and independently checked before training |
...
8. **Darcy v2 scientific protocol freeze:** `DARCY-FREEZE-001 / darcy-operator-ood-v2` is now reconciled as frozen-before-outcome. This closes protocol design only; the mandatory executable learned-model/configuration/budget freeze remains open and no v2 training/test run is authorized.
...
Scheduled production-health run `31817794439` checked `https://www.vertexed.app` against the deploy-relevant revision selected by the repository's Vercel path contract.

- expected deploy revision: `e2ecd19ed9816f8f36369c7dc0f38e39942ca73a`;
- `/api/health`: healthy, but immutable `revision` missing on all 3 attempts;
- unknown API route: expected 404 PASS;
- homepage: HTTP 200 PASS;
- malformed waitlist request: expected 400 PASS;
- logged-out `/api/ask`, `/api/user-content`, `/api/admin-status`: expected 401 PASS;
- untrusted cross-origin request: expected 403 PASS;
...
**Zero new major scientific experiment runs are authorized.** LAM is owner metadata + external-review only. IRIS/Percy/Project 2424/NPMS are source/live-state recovery first. Darcy's v2 scientific protocol is frozen, but execution remains blocked until its mandatory executable learned-model/configuration/compute freeze is committed before training. JEPA × time-series remains dormant. NeuroCAD research is deprioritized after its mechanism falsifier. VertexED is deployment certification only. Unused compute remains unused rather than being converted into low-information work.
