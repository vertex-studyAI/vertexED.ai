...
**Canonical registry:** `portfolio/portfolio.yaml` (schema v2; update in place, no competing master registry)  
...
| IRIS v0.2 | **VERIFIED mixed/negative; source PARTIALLY_RECOVERED / frontier BLOCKED** | exact canonical development trajectories + exact frozen adaptation-metric provenance still missing; seeds `1000–1029` forbidden; no approximate regeneration |
| NeuroCAD | **VERIFIED historical software; typed-parser mechanism FALSIFIED** | preserve `VALIDATION_DOMINANT`; new science needs a fresh benchmark |
...
| Project 2424 | **PARTIAL — WAVE-001 BASE RECOVERED / LATER OVERLAY + CANONICALIZATION BLOCKED** | historical bundle SHA `4c685af70d84052c026602ff7336a522c741d91fb480038e980c21f0bbc63ece`; `wave-001-push-ready@ff609f335f91297357b430a2531633fe111cd5a9`; clean quality gate PASS; 2,424 registry rows, only 24 source-backed packages, 0 independent reproductions, retained `RELEASE_REJECTED`. Current T2424 source-identity guard merged as `f60dafed1563ef51f73c39a4d4d0a392dadc0160`. Recover later dirty overlay and P2424↔T2424 migration provenance; never join by numeric suffix alone |
...
| VertexED Supabase | **PARTIAL controls verified / platform warnings open** | RLS/security metadata bounded-positive; leaked-password protection disabled and hosted PostgreSQL security update available; owner/platform review only |
...
| FinanceMeta source | **PARTIAL — 41-commit hardening lineage recovered / CI definition red** | `cursor/membership-security-supabase-fix@6dcc03710bb6adf9b4b722b308c40a0720bea61f`, 41 ahead / 0 behind `main@fbdd503223edc5b1780509720391083f485a4a85`; Actions `29641469740` fails with zero jobs because E2E env map duplicates `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `VITE_APP_URL`; connector writes return 403 |
| FinanceMeta Preview | **VERIFIED Preview only** | Vercel deployment `5501026657` for exact branch SHA succeeded as `Preview`; explicitly not production |
| FinanceMeta production | **BLOCKED_EXTERNAL** | live Supabase migrations/RLS/env/revision + multi-account journey unverified |
