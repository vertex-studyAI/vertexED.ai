...
## 3 — VERTEX-PROD-001 — P0 PRODUCT / PARTIAL

Source blocker is materially narrowed: PR `#397`, head `8cf6c65d1786358e902952c5dee4e1250be9948b`, makes production health fail closed when immutable identity is absent. Exact-head CI `31861346546` and PR monitor `31861346551` succeed. The PR remains unmerged and its PR-ref monitor is not exact production revision proof.
...
**Next action:** review #397; if accepted, merge/deploy that exact revision using existing authorized capacity, then require canonical production monitor exact-revision PASS. Only then run disposable-account signup/auth/onboarding/core-persistence/isolation/recovery/logout/admin-denial + cleanup. Do not auto-merge solely because CI is green.
