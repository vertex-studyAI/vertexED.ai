...
- On current control head `d5e9fcaa8de4e49b236b18ff7d3c515ed5f1ed6d`, both Vercel status contexts also report `build-rate-limit` failures. Do not infer that current control head is serving production.
...
**Zero new major scientific experiment runs are authorized right now.** Percy/Project 2424/IRIS/NPMS are source or live-state recovery first; Darcy v2 is protocol-frozen but still blocked on its mandatory executable implementation/config/budget freeze; JEPA×time-series is dormant design; NeuroCAD research is deprioritized after its mechanism falsifier; LAM is owner metadata + external review only; VertexED is deployment certification only. Unused compute capacity should remain unused rather than generate low-information experiments.
