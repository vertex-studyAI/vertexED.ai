...
try {
  if (cmd === 'doctor' || cmd === 'status') {
    ...
    console.log(JSON.stringify({
      ...
      counts: store.counts(),
      tasks: cmd === 'status' ? store.list(20) : undefined,
      integrity,
    }, null, 2));
    ...
    console.log(JSON.stringify({ paused: false, expiredRecovered: store.resumeStale() }));
  } else if (cmd === 'requeue-stale') {
    const taskId = take('--task-id');
    if (!taskId) throw new Error('--task-id required');
    const requeued = store.requeueStale(taskId);
    console.log(JSON.stringify({ taskId, requeued }));
    ...
    const onSignal = (signal) => {
      draining = true;
      logger.write('drain_requested', { signal });
    };
