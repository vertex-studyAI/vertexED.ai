...
export async function runWorkerLoop({
  ...
  while (!shouldStop()) {
    ...
    let heartbeat;
    try {
      // Keep the claim alive while waiting behind a stricter provider/class
      // limiter; otherwise the lease can expire and the same task can be
      // reclaimed while this worker is still waiting to execute it.
