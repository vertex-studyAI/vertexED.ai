...
function validateImages(imagesInput = []) {
  ...
  for (const img of imagesInput.slice(0, 10)) {
    ...
    out.push({ name: name || null, mime: mime || null, b64: b64 || null, url: url || null, caption: caption || null });
  }
...
function buildUserPrompt(data) {
  const {
    board, grade, subject, topics, format,
    difficulty, numQuestions, marks,
    criteria, anythingElse, priorPapers, images
  } = data;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const paper = result?.choices?.[0]?.message?.content;
