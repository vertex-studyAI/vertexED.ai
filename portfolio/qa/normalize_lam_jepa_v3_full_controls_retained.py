...
def main() -> None:
    ...
    train_eligibility = protocol.get("train_eligibility")
    validation_eligibility = protocol.get("validation_eligibility")
    require(isinstance(train_eligibility, dict), "train_eligibility block missing")
    require(isinstance(validation_eligibility, dict), "validation_eligibility block missing")
    train_eligible_rows = train_eligibility.get("eligible_rows")
    validation_eligible_rows = validation_eligibility.get("eligible_rows")
    require(train_eligible_rows == 1117, f"unexpected train eligible row count: {train_eligible_rows}")
    require(validation_eligible_rows == 295, f"unexpected validation eligible row count: {validation_eligible_rows}")
    ...
    normalized = {
        ...
        "protocol": protocol,
        # Compatibility aliases for the independent verifier. These are copied
        # verbatim from protocol.*_eligibility.eligible_rows; no experimental
        # value is recomputed or modified.
        "train_eligible_rows": train_eligible_rows,
