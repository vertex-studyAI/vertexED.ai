...
For each surface and each frozen seed, let $M(a)$ denote the official \texttt{EW\_F\_0.50} score of algorithm $a$ on that matched mission-seed pair, and define
\begin{align}
b &= \max\{M(\text{robust-z}), M(\text{persistence})\},\\
\Delta &= M(\text{Space-JEPA}) - b.
\end{align}
...
\begin{tabular}{@{}lcc@{}}
\toprule
Surface & Metric & Frozen rule \\
\midrule
\texttt{mission1-lite} & \texttt{EW\_F\_0.50} & mean $\Delta>0$, $\geq$4/5 wins \\
\texttt{mission2-lite} & \texttt{EW\_F\_0.50} & mean $\Delta>0$, $\geq$4/5 wins \\
Overall & -- & both surfaces pass \\
\bottomrule
\end{tabular}
\end{table}
