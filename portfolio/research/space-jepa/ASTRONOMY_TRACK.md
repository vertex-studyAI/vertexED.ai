...
The intended first data family is public multi-epoch photometry. Aadi Nair's reported transient can be used as a qualitative held-out case only after the method and thresholds are frozen; it must not be the sole evaluation set or a tuning target.
...
Primary retained metrics must be chosen to match the actual public benchmark/task before viewing held-out labels. For a labeled transient-ranking/classification dataset, report at least average precision and AUROC where meaningful, plus any task-native classification metric and a precision/recall operating point fixed from training/validation data. For forecasting, report a predeclared latent/photometric prediction metric on a held-out chronological block.
...
For PLAsTiCC specifically, promotion also requires evidence that its unblinded held-out labels were excluded from every pre-outcome development decision and an explicit manuscript limitation that the benchmark is simulated. If that freshness condition cannot be established, PLAsTiCC may still be reported as a development/external-characterization benchmark but not as the confirmatory astronomy result.
