...
`tests.log` reports two Node subtests: **2 passed, 0 failed**.
...
- `0.95` vs `0.60` and delta `0.35` are not summary arithmetic artifacts;
...
Not established:
- fresh source-checkout execution;
...
A clean `git clone` was attempted in the independent execution container, but that container could not resolve `github.com` via DNS and returned Git status `128` before checkout. Classification: **`REPRODUCTION_BLOCKED_EXECUTION_ENV_DNS`**. This is not a NeuroCAD test failure and is not evidence for or against the method.

## Relationship to v2 falsification

This audit verifies historical v1 bytes/metrics only. The separately frozen 2026-08-14 component ablation v2 supersedes the typed-parser causal interpretation on the reused diagnostic: direct extraction plus matched fail-closed validation reaches `1.00` and matches the current compiler, giving `VALIDATION_DOMINANT`. The v1 history remains immutable; the mechanism claim is narrowed by v2 rather than rewritten.
