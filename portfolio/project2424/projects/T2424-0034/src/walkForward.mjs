...
export function validateWalkForwardPrices(prices) {
...
export function logReturns(prices) {
  const clean = validateWalkForwardPrices(prices);
...
export function walkForwardBacktest(prices, options = {}) {
  ...
  if (!Number.isInteger(minTrain) || minTrain < 2) {
    throw new RangeError('minTrain must be an integer >= 2');
  }
  ...
  for (let targetIndex = lookback; targetIndex < returns.length; targetIndex += 1) {
    const feature = mean(returns.slice(targetIndex - lookback, targetIndex));
    ...
    // Critical no-lookahead boundary: the current target becomes training evidence
    // only after this row has been predicted, positioned, costed, and scored.
...
function maxDrawdown(equity) {
  if (!Array.isArray(equity) || equity.length === 0) {
    throw new TypeError('equity must be non-empty');
  }
  let peak = equity[0];
  let worst = 0;
...
function annualizedSharpe(returns, periodsPerYear = 252) {
...
export function summarizeWalkForward(rows) {
  if (!Array.isArray(rows) || rows.length === 0) {
    throw new TypeError('rows must be non-empty');
  }
