...
export function validateRecord(record, index = 0) {
  ...
  const predicted = abstained
    ? String(record.predicted ?? '').trim()
    : requiredText(record.predicted, `records[${index}].predicted`);
...
export function validateRecords(records) {
  ...
  for (const record of normalized) {
    ...
    if (conceptLanguages.has(key)) {
      throw new RangeError(`duplicate concept/language pair: ${record.conceptId}/${record.language}`);
    }
    conceptLanguages.add(key);
    conceptCounts.set(record.conceptId, (conceptCounts.get(record.conceptId) ?? 0) + 1);
  }
  for (const [conceptId, count] of conceptCounts) {
    if (count < 2) throw new RangeError(`concept ${conceptId} must appear in at least two languages`);
  }
...
export function classifyRecord(recordInput, { blindSpotConfidence = 0.8 } = {}) {
  ...
  const highConfidenceCorrect = !record.abstained && correct && record.confidence >= threshold;
  return { ...record, correct, highConfidenceWrong, highConfidenceCorrect };
}

function summarize(classified) {
  const total = classified.length;
  const answeredRecords = classified.filter((record) => !record.abstained);
  ...
  const meanConfidence = answered === 0
    ? null
    : answeredRecords.reduce((sum, record) => sum + record.confidence, 0) / answered;
  const brier = answered === 0
    ? null
    : answeredRecords.reduce(
        (sum, record) => sum + (record.confidence - Number(record.correct)) ** 2,
        0,
      ) / answered;
  const selectiveAccuracy = answered === 0 ? null : correct / answered;
  return {
    total,
    answered,
    correct,
    ...
    brier,
    calibrationGap:
      meanConfidence === null || selectiveAccuracy === null
        ? null
        : Math.abs(meanConfidence - selectiveAccuracy),
    highConfidenceWrong,
...
export function languageMetrics(records, options = {}) {
  ...
  return [...groups.entries()]
    ...
    .sort(
      (left, right) =>
        right.blindSpotRate - left.blindSpotRate || left.language.localeCompare(right.language),
    );
}
...
export function conceptBlindSpots(records, options = {}) {
  ...
  return [...groups.entries()]
    .map(([conceptId, group]) => {
      ...
      const highConfidenceCorrectLanguages = group
        .filter((record) => record.highConfidenceCorrect)
        .map((record) => record.language)
        .sort();
      const blindSpotLanguages = group
        .filter((record) => record.highConfidenceWrong)
        .map((record) => record.language)
        .sort();
      ...
      return {
        ...
        correctLanguages,
        highConfidenceCorrectLanguages,
        blindSpotLanguages,
        abstainedLanguages,
        crossLanguageMismatch:
          correctLanguages.length > 0 && group.some((record) => !record.correct && !record.abstained),
        crossLanguageBlindSpot:
          highConfidenceCorrectLanguages.length > 0 && blindSpotLanguages.length > 0,
      };
    })
    .sort(
      (left, right) =>
        Number(right.crossLanguageBlindSpot) - Number(left.crossLanguageBlindSpot) ||
...
export function compareLanguages(records, languageA, languageB, options = {}) {
  ...
  for (const [, pair] of pairs) {
    ...
    if (pair[a].highConfidenceWrong && pair[b].highConfidenceCorrect) blindSpotsAOnly += 1;
    if (pair[b].highConfidenceWrong && pair[a].highConfidenceCorrect) blindSpotsBOnly += 1;
  }
  ...
  return {
    languageA: a,
    languageB: b,
    matchedConcepts: pairs.length,
    accuracyA,
    accuracyB,
    accuracyGap: accuracyA - accuracyB,
    blindSpotsAOnly,
    blindSpotsBOnly,
  };
}
...
export function buildBenchmarkReport(records, options = {}) {
  ...
  return {
    ...
    languages,
    concepts,
    crossLanguageMismatchCount: concepts.filter((entry) => entry.crossLanguageMismatch).length,
    crossLanguageBlindSpotCount: concepts.filter((entry) => entry.crossLanguageBlindSpot).length,
    claimBoundary:
      'deterministic evaluation of supplied aligned records only; no translation-quality, semantic-equivalence, or real-model capability claim',
  };
}
