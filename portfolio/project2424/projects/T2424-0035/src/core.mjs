...
export function validateLearningCurve(rows) {
  if (!Array.isArray(rows) || rows.length < 8) throw new TypeError("learning curve must contain at least 8 rows");
  let previousStep = -Infinity;
  return rows.map((row, index) => {
    if (!row || !Number.isFinite(row.step) || row.step <= previousStep) throw new RangeError(`rows[${index}].step must be finite and strictly increasing`);
    ...
    return { step: Number(row.step), trainAccuracy: Number(row.trainAccuracy), evalAccuracy: Number(row.evalAccuracy) };
  });
}
...
function firstPersistentThreshold(rows, field, threshold, persistence) {
  for (let start = 0; start <= rows.length - persistence; start += 1) {
    ...
    for (let offset = 0; offset < persistence; offset += 1) {
      if (rows[start + offset][field] < threshold) { passes = false; break; }
    }
...
export function analyzeGrokking(rows, options = {}) {
  ...
  [trainThreshold, evalThreshold, maxEvalAtMemorization].forEach((value, index) => validateAccuracy(value, ["trainThreshold", "evalThreshold", "maxEvalAtMemorization"][index]));
  ...
  const smoothed = clean.map((row, index) => ({ step: row.step, trainAccuracy: trainSmoothed[index], evalAccuracy: evalSmoothed[index] }));
  ...
  const delaySteps = memorizationStep === null || generalizationStep === null ? null : generalizationStep - memorizationStep;
  const evalAtMemorization = valueAtOrAfterStep(smoothed, "evalAccuracy", memorizationStep);
  const grokkingDetected = memorizationStep !== null && generalizationStep !== null && delaySteps >= minDelaySteps && generalizationStep > memorizationStep && evalAtMemorization !== null && evalAtMemorization <= maxEvalAtMemorization;
  ...
  return { verdict, grokkingDetected, memorizationStep, generalizationStep, delaySteps, evalAtMemorization, thresholds: { trainThreshold, evalThreshold, minDelaySteps, maxEvalAtMemorization, smoothingWindow, persistence }, smoothed };
}
...
export function generateMatchedControlCurve(options = {}) {
  ...
  return generateSyntheticCurve({ ...options, trainMidpoint: midpoint, evalMidpoint: midpoint + (options.evalLag ?? 250) });
}
