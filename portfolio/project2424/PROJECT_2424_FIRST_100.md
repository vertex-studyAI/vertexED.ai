...
## Identity conflict requiring repair

`portfolio/project2424/projects/T2424-0049/` currently contains **Project24 Render**, merged as `6581a39539267c85b247aa30363d5285daef0173` after exact-head CI `31414274233`. However, the canonical `FIRST_100_QUEUE.ndjson` maps `T2424-0049` / rank 42 to **Multiphase Porous JEPA**.

Therefore Project24 Render is **not counted** as a First-100 package until its identity is reconciled without overwriting the canonical queue entry. Treat it as a tested standalone portfolio tool with an ID collision, not as an eighth First-100 completion.

## Verified or active but deliberately unmerged First-100 work

These do **not** increase the seven-package merged count. Their PR boundaries must be respected rather than bypassed simply because a test run is green.
