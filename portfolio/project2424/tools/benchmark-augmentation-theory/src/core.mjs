...
export function validateBenchmarkCase(record) {
  ...
  return {
    id,
    signal: finiteNumber(record.signal, 'signal'),
    shortcut: finiteNumber(record.shortcut, 'shortcut'),
    label: binaryLabel(record.label),
  };
}
...
export function createShortcutBenchmark({ size = 100 } = {}) {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    const signal = label * magnitude;
    const shortcut = label * (1 + ((index * 11) % 13) / 10);
    return { id: `case-${String(index).padStart(3, '0')}`, signal, shortcut, label };
  });
}
...
export function augmentBenchmark(cases, augmentations) {
  ...
  if (!Array.isArray(augmentations) || augmentations.length === 0) {
    throw new TypeError('augmentations must be a non-empty array');
  }
  ...
  for (const augmentation of augmentations) {
    ...
    for (const record of normalized) {
      const transformed = validateBenchmarkCase({
        ...augmentation.transform({ ...record }),
        id: `${record.id}::${name}`,
      });
      if (transformed.label !== record.label) {
        throw new Error(`augmentation ${name} changed label for ${record.id}`);
      }
...
export function rankModels(scores) {
  ...
  return [...scores].sort((left, right) => {
    const accuracyDelta = finiteNumber(right.accuracy, 'accuracy') - finiteNumber(left.accuracy, 'accuracy');
...
export function compareModels({ cases, augmentations, models }) {
  ...
  const results = models.map((model) => {
    ...
    return {
      ...
      baseAccuracy,
      augmentedAccuracy,
      robustnessDelta: augmentedAccuracy - baseAccuracy,
    };
  });
...
export const robustSignalModel = Object.freeze({
  name: 'signal-model',
  predict(record) {
    return record.signal >= 0 ? 1 : -1;
  },
});

export const shortcutModel = Object.freeze({
  name: 'shortcut-model',
  predict(record) {
    return record.shortcut >= 0 ? 1 : -1;
  },
});

export const flipShortcutAugmentation = Object.freeze({
  name: 'flip-shortcut',
  transform(record) {
    return { ...record, shortcut: -record.shortcut };
  },
});

export const neutralSignalScaleAugmentation = Object.freeze({
  name: 'scale-signal',
  transform(record) {
    return { ...record, signal: record.signal * 2 };
  },
});
