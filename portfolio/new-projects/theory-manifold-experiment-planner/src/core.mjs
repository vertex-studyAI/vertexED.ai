...
function validateCandidate(candidate, index = 0) {
  if (!candidate || typeof candidate.id !== "string" || candidate.id.trim().length === 0) {
    throw new TypeError(`candidates[${index}].id must be non-empty`);
  }
  if (typeof candidate.family !== "string" || candidate.family.trim().length === 0) {
    throw new TypeError(`candidates[${index}].family must be non-empty`);
  }
  if (!Number.isFinite(candidate.costHours) || candidate.costHours <= 0) {
    throw new RangeError(`candidates[${index}].costHours must be finite and > 0`);
  }
  return {
    ...
    costHours: Number(candidate.costHours),
    dependenciesComplete: candidate.dependenciesComplete !== false
  };
}
...
export function acquisitionScore(candidate, options = {}) {
  ...
  [explorationWeight, noveltyWeight, costExponent, repeatFamilyPenalty].forEach((value, index) => {
    if (!Number.isFinite(value) || value < 0) {
      throw new RangeError(["explorationWeight", "noveltyWeight", "costExponent", "repeatFamilyPenalty"][index] + " must be finite and >= 0");
    }
  });
  if (!Number.isInteger(familySelections) || familySelections < 0) {
    throw new RangeError("familySelections must be a non-negative integer");
  }
  ...
  const costPenalty = clean.costHours ** costExponent;
  const diversityPenalty = 1 + repeatFamilyPenalty * familySelections;
  return benefit / (costPenalty * diversityPenalty);
}
...
export function rankExperiments(candidates, options = {}) {
  ...
  return clean
    .map((candidate) => ({
      ...candidate,
      score: acquisitionScore(candidate, options)
    }))
    .sort((left, right) => {
...
export function selectExperimentBatch(candidates, options = {}) {
  ...
  while (selected.length < batchSize) {
    const feasible = remaining
      .filter((candidate) => candidate.dependenciesComplete && usedHours + candidate.costHours <= budgetHours)
      .map((candidate) => ({
        candidate,
        score: acquisitionScore(candidate, {
          ...options,
          repeatFamilyPenalty,
          familySelections: familyCounts.get(candidate.family) ?? 0
        })
      }))
      .sort((left, right) => {
        if (right.score !== left.score) return right.score - left.score;
    ...
    const index = remaining.findIndex((candidate) => candidate.id === winner.candidate.id);
...
export function updateCandidateFromEvidence(candidate, observedValue, evidenceWeight = 0.5) {
  ...
  if (!Number.isFinite(evidenceWeight) || evidenceWeight <= 0 || evidenceWeight > 1) {
    throw new RangeError("evidenceWeight must be in (0, 1]");
  }
  const expectedValue = (1 - evidenceWeight) * clean.expectedValue + evidenceWeight * observation;
  const uncertainty = clean.uncertainty * (1 - 0.75 * evidenceWeight);
  return {
    ...clean,
    expectedValue,
    uncertainty
...
export function buildDecisionLedger(candidates, options = {}) {
  const ranked = rankExperiments(candidates, options);
