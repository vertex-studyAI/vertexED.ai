...
export function expectedCalibrationError(records, binCount = 10) {
  const clean = validatePredictions(records);
  const bins = calibrationBins(clean, binCount);
...
export function selectiveRiskAtCoverage(records, targetCoverage) {
  ...
  const targetAccepted = Math.ceil(targetCoverage * curve.length);
  return curve[Math.max(0, targetAccepted - 1)];
}
...
export function abstentionReport(records, confidenceThreshold) {
  ...
  const rejected = clean.length - accepted.length;
...
export function summarizeTrust(records, options = {}) {
  ...
  if (!Array.isArray(coverages) || coverages.length === 0) {
    throw new TypeError("coverages must be non-empty");
  }
  return {
