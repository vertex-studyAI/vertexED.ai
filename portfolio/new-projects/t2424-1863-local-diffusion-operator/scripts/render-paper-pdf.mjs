...
<head>
<meta charset="utf-8">
<title>T2424-1863 reproducible negative-result manuscript</title>
<style>
...
async function main() {
  const [manuscriptPath, figurePath, outputPath] = process.argv.slice(2);
  if (!manuscriptPath || !figurePath || !outputPath) {
    throw new Error('Usage: node scripts/render-paper-pdf.mjs <MANUSCRIPT.md> <figure.svg> <paper.pdf>');
  }

  const [markdown, figureSvg] = await Promise.all([
