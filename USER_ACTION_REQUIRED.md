...
7. **The Bu1LD deployment/runtime gate:** repository source access is already available and exact current-main CI passed at `daa80c1124b2a6d7d09b7669e04d29e50cffcbbe`; do **not** recreate source work. The remaining external/owner gate is the real deployment/runtime surface. Diagnose/fix the exact Cloudflare `release:check` failure, rerun the intended SHA, prove immutable served revision, then certify Supabase RLS/role boundaries, hydration and the seven-role authenticated journey.

8. **IRIS remaining provenance:** substantial source/config/environment lineage is checksum-recovered on control `main`, but the frozen frontier is still blocked on the exact canonical development trajectories and exact frozen adaptation-metric source identity. Recover those exact provenance edges or retain `PROTOCOL_BLOCKED`; seeds `1000–1029` remain forbidden.
