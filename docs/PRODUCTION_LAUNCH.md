...
2. Review the target project and migration plan before running `npx supabase db push`; `supabase/migrations/` is the application's schema source of truth, not evidence that a shared remote database matches it. Run the read-only `scripts/supabase-security-audit.sql` preflight and reconcile divergent migration histories before any push.
   - Export a **sanitized, read-only remote migration-version list** (JSON array/rows or one 8- or 14-digit remote version per line) and run `npm run db:ledger:check -- --ledger <path>`. The checker requires every checked-in VertexED migration to exist remotely, reports shared-project/remote-only versions without treating them as automatic corruption, and fails if a required VertexED version is missing beneath later remote migrations. It never connects to or mutates Supabase.
   - Do not pipe the human table from `supabase migration list` directly into the checker: a two-column local/remote table can hide which side owns a version. Use a remote-only export from the migration ledger instead.
