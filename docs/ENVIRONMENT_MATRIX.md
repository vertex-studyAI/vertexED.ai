...
| `HEALTH_READINESS_TOKEN` | Server secret | Required for production readiness detail | When set, `/api/health?readiness=1` returns capability `checks` only if the request includes matching `x-vertexed-readiness-token`. On Vercel production (`VERCEL_ENV=production`), unset token fails closed and redacts `checks`. Smoke scripts read the same env var. | Operator probes / public scanners | Leave unset only for local/dev; set before exposing production readiness detail. |
