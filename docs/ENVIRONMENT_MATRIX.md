...
| `HEALTH_READINESS_TOKEN` | Server secret | Recommended in production | When set, `/api/health?readiness=1` returns capability `checks` only if the request includes matching `x-vertexed-readiness-token`. Smoke scripts read the same env var. | Operator probes / public scanners | Leave unset only for local/dev; set before exposing production readiness detail. |
