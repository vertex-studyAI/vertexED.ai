...
| `HEALTH_READINESS_TOKEN` | Server secret | Required for production readiness detail | When set, `/api/health?readiness=1` returns capability `checks` only if the request includes matching `x-vertexed-readiness-token`. When unset, production-like hosts (`VERCEL_ENV=production` or `NODE_ENV=production`) redact `checks`. Smoke scripts read the same env var. | Operator probes / public scanners | Leave unset only for local/dev; set before exposing production readiness detail. |
| `ALLOW_LEGACY_INVITE_TOKENS` | Server | No (`1` to re-enable, `0` to force off) | Plaintext `invite_token` waitlist lookups for pre-digest invite links. Default: allowed outside production, denied in production. | Unknown | Production stays on hashed invite tokens only unless an explicit migration window sets `1`. |
| `TRUST_X_FORWARDED_FOR` | Server | No (`1` to trust) | When `1`, `getClientIp` may use the first `X-Forwarded-For` hop. Otherwise prefers platform headers (`x-vercel-forwarded-for`, `x-real-ip`). | Unknown | Spoofable client IPs can dilute rate limits if enabled behind an untrusted proxy. |
...
| `RESEND_API_KEY` | Server secret | Recommended | Waitlist approval email | Unknown | Approval still issues a one-time token; when email delivery succeeds the invite URL is omitted from the admin JSON response. Without Resend, the URL is returned to the authorized admin only. Recipient addresses and invite links are not logged. |
...
| `VITE_ALLOW_CLIENT_ADMIN_FALLBACK` | Client, public | No (`1` to enable) | Dev-only: allow `VITE_ADMIN_EMAILS` client allowlist when `/api/admin-status` denies. Default off. | Unknown | Without `1`, client admin UI never elevates from the email allowlist alone. |
| `VITE_SKIP_WAITLIST_GATE` | Client, public | No (`1` to enable) | Dev-only: skip ProtectedRoute waitlist gate when `import.meta.env.DEV`. Default off. | Unknown | Accidental production builds must not ship with `1`. |
