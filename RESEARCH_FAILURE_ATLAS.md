...
| Darcy v2 — pre-outcome integrity | frozen v2 outcomes must remain unseen until all declared implementation/environment/split/model prerequisites are immutable | original PR #360 engineering invariant test computed solver-error metrics on frozen OOD-C seed `320007`; no learned training, promotion-gate inspection or claimed v2 result occurred, but the no-outcome-peek boundary was still crossed | no v2 scientific outcome may be claimed from that path; parent bounded result is unchanged; process defect remains evidence | safety correction `daf548fa9b3953c3d7e188191588a84a04c98093` replaces the metric fixture with a hand-constructed non-protocol field and records generator ambiguities as pre-training blockers; training remains locked |
...
7. Any accidental pre-outcome test/metric access is recorded as an integrity failure even when it does not change a scientific verdict; subsequent work must remove the leak and preserve the event.
