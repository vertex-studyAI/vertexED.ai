...
| **Darcy v2 pre-outcome procedure** | frozen v2 requires implementation/environment/split/model-budget closure before ID/OOD scientific outcome evaluation | PR #360 unit test used frozen OOD-C seed `320007` with the reference/control evaluator, computing solver error metrics before all pretraining blockers were closed | **procedural contamination only; the computation is invalid/non-evidence and cannot support or refute H1/H2** | seed/case remains frozen and unchanged; values are not used to tune/promote/exclude anything; `main@daf548fa9b3953c3d7e188191588a84a04c98093` replaces the solver-metric unit input with a non-protocol fixture, records generator provenance and adds unresolved pretraining interpretation locks before any future outcome run |
...
2. Software/procedural bug → document, narrowly repair, quarantine contaminated evidence, rerun only under the unchanged frozen scientific identity when the original protocol permits it.
...
7. An accidental pre-outcome computation never becomes evidence merely because CI passed; preserve the case and gates rather than editing the test set after the fact.
