...
|---|---|---|
| Control plane | **VERIFIED source** | canonical `vertex-studyAI/vertexED.ai/main` observed at `9abedd1dfc297c876e8e24fc01f223033ac0eca6` after a forward cleanup commit removed an accidental one-line permission-probe sentinel. No reset or history rewrite occurred. This convergence ledger work is isolated on `chatgpt/final-convergence-20260815-0842` and is not auto-merged |
...
| VertexED production | **BLOCKED — EXACT SERVED REVISION / AUTHENTICATED JOURNEY** | latest scheduled monitor `31860931665` ran on `main@8d8c0bf215c59668519b2ed7ef7fad35f185f53a` and failed after three bounded attempts because live `/api/health` was healthy but omitted revision identity; expected deploy revision remained `e2ecd19ed9816f8f36369c7dc0f38e39942ca73a`. Homepage, unknown-route 404, malformed waitlist 400, logged-out ask/user-content/admin 401 and untrusted-origin 403 checks passed. Artifact `9240538693`, SHA-256 `205b0d17ba3c1899addd558f2c0615ab32148af43a5f8fb6a55a510f4eb66394`. Source already emits immutable revision when deployed with the existing build identity path; do not duplicate the source fix. Exact served deployment identity + monitor PASS + authenticated disposable-account journey remain required |
...
3. `VERTEX-PROD-001` — deploy/identify the exact existing runtime revision using already-authorized capacity, make immutable revision monitor PASS, then authenticated disposable-account golden journey.
...
7. Darcy v2 — close B3/B4 + interpretation/environment/hardware/budget locks only; **no training or ID/OOD outcome access**.
