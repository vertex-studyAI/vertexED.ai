VertexED AI is an all-in-one study workspace designed to help students plan, learn, and review with less friction.
...
- Supabase (auth + data)
- Serverless / edge functions (API route scripts and Vercel functions)
...
Copy `.env.example` to `.env.local` and fill in the required keys before running auth or AI features.
...
- AI request routing through server handlers
- Rate limiting and validation on every API endpoint
...
- Ambient particle background with reduced-motion support

If you add new screens, keep them token-driven, keyboard accessible, and responsive across mobile and desktop.
