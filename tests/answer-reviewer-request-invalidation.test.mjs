...
test('answer reviewer invalidates stale review requests on reset, account change, and unmount', () => {
  ...
  assert.match(source, /const invalidateReviewRequest = useCallback\(\(\) => \{[\s\S]*reviewRequestIdRef\.current \+= 1;[\s\S]*setLoading\(false\);[\s\S]*\}, \[\]\);/);
  assert.match(source, /useLayoutEffect\(\(\) => \{[\s\S]*currentReviewAccountIdRef\.current !== nextAccountId[\s\S]*currentReviewAccountIdRef\.current = nextAccountId;[\s\S]*invalidateReviewRequest\(\);[\s\S]*\}, \[user\?\.id, invalidateReviewRequest\]\);/);
...
test('answer reviewer gates post-await work and stale saves on the current request identity', () => {
  ...
  assert.match(source, /const requestAccountId = currentReviewAccountIdRef\.current;/);
