...
// The repaired branch must have zero materialized package↔queue identity
// mismatches. Future intentional exceptions must be explicit and must match
// the observed conflict set exactly rather than accumulating silently.
...
test('Project 2424 materialized package identities match the canonical frozen queue', async () => {
  const rows = await loadQueue();
