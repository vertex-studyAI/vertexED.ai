...
} from "../portfolio/new-projects/grokking-agent/src/core.mjs";
...
test("curve validation fails closed on malformed metrics and step ordering", () => {
  ...
  assert.throws(
    () => validateLearningCurve(valid.map((row, index) => index === 5 ? { ...row, trainAccuracy: 1.2 } : row)),
    /\[0, 1\]/
  );
  assert.throws(
    () => validateLearningCurve(valid.map((row, index) => index === 5 ? { ...row, step: valid[4].step } : row)),
    /strictly increasing/
  );
});
