...
} from "../portfolio/project2424/projects/T2424-0026/src/core.mjs";
...
test("frozen Rule-110 experiment satisfies the predeclared minimum gate", () => {
  const result = simulateCounterfactual(deterministicSeedState(81), {
    rule: 110,
    steps: 40,
    intervention: { time: 10, index: 40, mode: "flip" }
  });
  assert.ok(result.divergence.filter((row) => row.time < 10).every((row) => row.hammingDistance === 0));
  assert.deepEqual(causalConeViolations(result), []);
  assert.equal(summarizeCounterfactual(result).causalConeViolations, 0);
});
