...
test('cross-language blind spot requires high-confidence correct reference evidence', () => {
  ...
  assert.deepEqual(c1.highConfidenceCorrectLanguages, ['en', 'fr']);
  assert.equal(c2.crossLanguageMismatch, true);
  assert.equal(c2.crossLanguageBlindSpot, false);
});

test('language metrics separate coverage, selective accuracy, calibration and blind spots', () => {
  ...
  assert.ok(metrics.es.brier > metrics.en.brier);
  assert.ok(metrics.es.calibrationGap > metrics.en.calibrationGap);
});

test('paired comparison reports directional high-confidence blind-spot asymmetry', () => {
...
test('benchmark report distinguishes mismatch count from strict blind-spot count', () => {
  const report = buildBenchmarkReport(records);
  assert.equal(report.crossLanguageMismatchCount, 2);
...
test('evaluation is invariant to input row order', () => {
  const forward = buildBenchmarkReport(records);
  const reversed = buildBenchmarkReport([...records].reverse());
  assert.deepEqual(reversed, forward);
});

test('duplicate alignment keys, singleton concepts and malformed confidences fail closed', () => {
  assert.throws(
    () => validateRecords([...records, { ...records[0], itemId: 'duplicate-key' }]),
  ...
  assert.throws(
    () =>
      validateRecords([
        ...records,
        { itemId: 'solo-de', conceptId: 'solo', language: 'de', expected: 'x', predicted: 'x', confidence: 0.9 },
      ]),
    /must appear in at least two languages/,
  );
  assert.throws(() => classifyRecord({ ...records[0], confidence: 1.1 }), /\[0, 1\]/);
  assert.throws(() => compareLanguages(records, 'en', 'en'), /must differ/);
});

test('raising the threshold removes the injected blind spot without erasing correctness mismatch', () => {
  const report = buildBenchmarkReport(records, { blindSpotConfidence: 0.99 });
  assert.equal(report.crossLanguageBlindSpotCount, 0);
  assert.equal(report.crossLanguageMismatchCount, 2);
});
