...
test('Project 2424 frozen First-100 registry has exactly 100 unique identities', async () => {
...
test('every repository Project 2424 directory is represented exactly once in the source identity manifest', async () => {
  ...
  assert.deepEqual(manifestIds, projectDirectories, 'directory set and source identity manifest must match exactly');
  assert.equal(manifest.represented_project_directory_count, projectDirectories.length);
  assert.equal(manifest.truth_boundary.full_2424_registry_recovered, false);
...
test('source identity manifest names and paths match the frozen First-100 registry', async () => {
  const registry = await loadRegistry();
  const manifest = await loadManifest();
...
test('known repaired collisions remain bound to their frozen registry identities', async () => {
  const manifest = await loadManifest();
