...
export function renderMarkdown(markdown, figures = {}) {
  ...
  while (index < lines.length) {
    ...
    if (image) {
      output.push(`<figure>${normalizePrintText(figures[image[2]] || '')}<figcaption>${inline(image[1])}</figcaption></figure>`);
      index += 1;
      continue;
    }
...
export function documentHtml(markdown, figures = {}) {
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
<title>T2424-0025 robust weighted readouts bounded manuscript</title>
<style>
...
<body>
${renderMarkdown(markdown, figures)}
</body>
</html>`;
}

async function main() {
  const [manuscriptPath, figure1Path, figure2Path, outputPath] = process.argv.slice(2);
  if (!manuscriptPath || !figure1Path || !figure2Path || !outputPath) {
    throw new Error('Usage: node scripts/render-t2424-0025-paper-pdf.mjs <MANUSCRIPT.md> <figure1.svg> <figure2.svg> <paper.pdf>');
  }

  let markdown = await readFile(manuscriptPath, 'utf8');
  const figures = {
    figure1: await readFile(figure1Path, 'utf8'),
    figure2: await readFile(figure2Path, 'utf8'),
  };
  ...
  try {
    ...
    await page.setContent(documentHtml(markdown, figures), { waitUntil: 'load' });
    await page.pdf({
      path: outputPath,
      format: 'A4',
