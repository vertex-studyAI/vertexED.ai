...
export function resolveBuildRevision({ env = process.env, runGit = defaultRunGit } = {}) {
  ...
  if ((source.present && !source.revision) || (vercel.present && !vercel.revision)) return null;
  if (source.revision && vercel.revision && source.revision !== vercel.revision) return null;
...
export async function writeBuildRevisionModule({ outputPath = DEFAULT_OUTPUT, env = process.env, runGit = defaultRunGit, required = requiresImmutableBuildRevision(env) } = {}) {
  const revision = resolveBuildRevision({ env, runGit });
  if (!revision && required) {
    throw new Error('Unable to resolve an immutable Git revision for this deploy-relevant build. Refusing to produce an unverifiable deployment artifact.');
  }
  const contents = ['// Generated by scripts/generate-build-revision.mjs. Do not edit by hand.', `export const BUILD_REVISION = ${revision ? JSON.stringify(revision) : 'null'};`, ''].join('\n');
...
async function run() {
  ...
  console.log(revision ? `[build-revision] stamped ${revision}` : '[build-revision] no immutable revision available; wrote null local-development stamp');
}
