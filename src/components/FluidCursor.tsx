...
export default function FluidCursor(props: Props) {
  ...
  useEffect(() => {
    ...
    const config: any = {
      ...
      DENSITY_DISSIPATION: merged.densityDissipation,
      VELOCITY_DISSIPATION: merged.velocityDissipation,
      PRESSURE: merged.pressure,
      PRESSURE_ITERATIONS: merged.pressureIterations,
      CURL: merged.curl,
      SPLAT_RADIUS: merged.splatRadius,
      SPLAT_FORCE: merged.splatForce,
      SHADING: merged.shading,
      COLOR_UPDATE_SPEED: merged.colorUpdateSpeed,
      PAUSED: false,
      BACK_COLOR: merged.backColor,
      TRANSPARENT: merged.transparent,
    };

    // ---- WebGL helpers (ported from your Vue implementation) ----
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      function getSupportedFormat(
        glLocal: WebGLRenderingContext | WebGL2RenderingContext,
        internalFormat: number,
        format: number,
        type: number,
      ) {
        if (!supportRenderTextureFormat(glLocal, internalFormat, format, type)) {
          if ("drawBuffers" in glLocal) {
            const gl2 = glLocal as WebGL2RenderingContext;
      ...
      function supportRenderTextureFormat(
        ...
        const texture = glLocal.createTexture();
        if (!texture) return false;

        glLocal.bindTexture(glLocal.TEXTURE_2D, texture);
        glLocal.texParameteri(glLocal.TEXTURE_2D, glLocal.TEXTURE_MIN_FILTER, glLocal.NEAREST);
        glLocal.texParameteri(glLocal.TEXTURE_2D, glLocal.TEXTURE_MAG_FILTER, glLocal.NEAREST);
        glLocal.texParameteri(glLocal.TEXTURE_2D, glLocal.TEXTURE_WRAP_S, glLocal.CLAMP_TO_EDGE);
        glLocal.texParameteri(glLocal.TEXTURE_2D, glLocal.TEXTURE_WRAP_T, glLocal.CLAMP_TO_EDGE);
        glLocal.texImage2D(glLocal.TEXTURE_2D, 0, internalFormat, 4, 4, 0, format, type, null);

        const fbo = glLocal.createFramebuffer();
        if (!fbo) return false;

        glLocal.bindFramebuffer(glLocal.FRAMEBUFFER, fbo);
        glLocal.framebufferTexture2D(glLocal.FRAMEBUFFER, glLocal.COLOR_ATTACHMENT0, glLocal.TEXTURE_2D, texture, 0);
        const status = glLocal.checkFramebufferStatus(glLocal.FRAMEBUFFER);
        return status === glLocal.FRAMEBUFFER_COMPLETE;
      }
    ...
    // Helper shader utilities (kept minimal — compiled exactly like original)
    ...
    function getUniforms(program: WebGLProgram | null) {
      if (!program) return {} as Record<string, WebGLUniformLocation | null>;
    ...
    // --- The heavy "step" & "render" functions are direct ports (kept intact) ---
    ...
    function splat(x: number, y: number, dx: number, dy: number, color: ColorRGB) {
      ...
      if (splatProgram.uniforms.color) {
        // note: velocity splat uses dx/dy channels
    ...
    document.body.addEventListener("mousemove", firstMouseMoveHandler);
    window.addEventListener("mousedown", onMouseDown);
    window.addEventListener("mousemove", onMouseMove);
    document.body.addEventListener("touchstart", onTouchStartFirst);
    ...
    return () => {
      ...
      document.body.removeEventListener("mousemove", firstMouseMoveHandler as any);
      document.body.removeEventListener("touchstart", onTouchStartFirst);
  ...
  return (
    <div className={cn("pointer-events-none fixed left-0 top-0 z-50 w-full h-full", props.className)}>
