...
type QuizResult = {
  ...
  isCorrect?: boolean;
  type?: string;
  scoreStatus?: "MEASURED" | "EVIDENCE_LINKED" | "PROVISIONAL";
  confidence?: number;
...
export default function NotetakerQuiz(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    setAdditionalInfo((current) => current.trim() || [
      ...
      `Recent measured attempts average ${Math.round(target.avgPercent)}% across ${target.attempts} attempt${target.attempts === 1 ? '' : 's'}.`,
  ...
  const handleSubmitQuiz = async () => {
    ...
    const localResults: QuizResult[] = generatedQuestions.map((q) => {
      ...
      if (q.type === "multiple_choice") {
        ...
        const validatedAnswerKey = q.provenance?.source === "official-resource"
          && q.provenance?.answerKeyStatus === "validated";
        return {
          id,
          selected: user,
          ...
          scoreStatus: validatedAnswerKey ? "MEASURED" : "EVIDENCE_LINKED",
          confidence: 1,
          humanReviewRequired: !validatedAnswerKey,
          escalationReason: validatedAnswerKey
            ? null
            : "The answer key was generated by AI and has not been validated against an official source.",
          type: q.type,
        };
      }
      return {
    ...
    const recordMeasuredResults = (results: QuizResult[]) => {
      for (const result of results.filter((item) => item.scoreStatus === "MEASURED")) {
        ...
        recordWeakness({
          ...
          verification: {
            ...
            reference: "Validated official-resource answer key",
          },
        });
      }
    };
    ...
    try {
      ...
      const merged: QuizResult[] = localResults.map((r): QuizResult => {
        ...
        return {
          ...
          scoreStatus: g.scoreStatus === "EVIDENCE_LINKED" ? "EVIDENCE_LINKED" : "PROVISIONAL",
      ...
      const totalScore = scoredGuidance.reduce((sum, r) => sum + (Number(r.score) || 0), 0);
  ...
  const uploadAudioBlob = async (blob: Blob) => {
    ...
    try {
      ...
      // The API accepts a fixed allow-list. Custom labels remain a UI-only
      // preference so they cannot become prompt instructions through uploads.
      fd.append("noteFormat", format);
  ...
  const accuracy = useMemo(() => {
    ...
    const scoredGuidance = quizResults.filter((result) => result.scoreStatus !== "PROVISIONAL");
    const totalScore = scoredGuidance.reduce((sum, r) => sum + (Number(r.score) || 0), 0);
    const totalMax = scoredGuidance.reduce((sum, r) => sum + (Number(r.maxScore) || 0), 0);
            ...
            {adaptiveTarget && (
              <div className="mb-5 rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4" role="status">
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-amber-700 dark:text-amber-300">
                  Based on your measured quiz results
                </p>
                            ...
                            {quizSubmitted && res && (
                              <div className="mt-3 space-y-1 text-sm">
                                {typeof res.score !== "undefined" && (
                                  <div>
                                    {res.scoreStatus === "MEASURED" ? "Measured score" : "Practice score guidance"}: <strong>{res.score}/{res.maxScore}</strong>
                                  </div>
                                )}
              ...
              {quizSubmitted && (
                <div className="mt-4 space-y-3">
                  ...
                  {quizResults?.some((result) => result.feedback || result.scoreStatus === "PROVISIONAL") && (
                    <AiFeedbackControls capability="quiz" />
                  )}
                </div>
              )}
            </div>
