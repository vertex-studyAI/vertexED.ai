/*
AIChatbot_FullRedo.jsx — simplified visuals (fixed)
- Moved `handleWidget` above the returned JSX so it's available when buttons render.
- Kept all original structure and behavior exactly as requested (no other file changes).
- Minor safety/robustness tweaks: small null guards and consistent ordering.
*/
...
export default function AIChatbotFullRedo() {
  ...
  const [messages, setMessages] = useState(() =>
    storage.get("messages_" + (storage.get("active_chat", null) || "root"), [])
  );
  ...
  useEffect(() => {
    // scroll to bottom on new messages or when widgets open
    try {
  ...
  // ---------- MOVED handleWidget here so it exists for JSX handlers ----------
...
3) Question 3?`,
  ...
  return (
    <div
      ...
      style={{
        background:
          "radial-gradient(ellipse at 10% 80%, rgba(64,85,102,0.12), transparent 8%), radial-gradient(circle at 92% 20%, rgba(26,35,52,0.12), transparent 8%), linear-gradient(180deg, #08101a 0%, #081018 100%)",
      }}
    >
      ...
      <div className="max-w-6xl mx-auto px-8 py-10">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-white/8 to-white/3 flex items-center justify-center text-sm font-semibold">
              VA
            </div>
            <div>
            ...
            <div className="rounded-2xl p-4 bg-white/4 border border-white/6 backdrop-blur-md h-[80vh] flex flex-col">
              ...
              <div className="flex-1 overflow-auto">
                ...
                <div className="space-y-3">
                  {chats.length === 0 && <div className="px-2 text-white/60">No chats yet — create a new chat to get started. Chats will appear here.</div>}
                  ...
                  {chats.map((c) => (
                    <div key={c.id} className="p-3 rounded-lg bg-white/3 border border-white/6 flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <button onClick={() => openChat(c.id)} className="font-medium text-left">
                            {c.title}
                          </button>
                      ...
                      <div className="flex items-center space-x-2">
                        <button onClick={() => removeChat(c.id)} className="p-2 rounded-md hover:bg-white/6">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </div>
              ...
              <div className="mt-4 text-xs text-white/60 flex items-center justify-between">
                <div>Autosave: on</div>
                <div className="flex items-center space-x-2">
                  <button onClick={clearMessages} className="px-3 py-1 rounded-md bg-white/5">
                    Clear
                  </button>
                </div>
              </div>
            ...
            <div className="rounded-2xl p-6 bg-white/6 border border-white/6 backdrop-blur-md h-[80vh] flex flex-col">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center space-x-3 text-sm text-white/60">
                  ...
                  <div className="text-xs">{activeChatId ? "Chat: " + String(activeChatId).slice(-6) : "No chat open"}</div>
                </div>
              </div>
              ...
              <div className="mt-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex items-center space-x-2">
                    <button onClick={() => { setUserInput(""); }} className="px-4 py-2 rounded-md bg-white/5">
                      Clear
                    </button>
      ...
      <div className={`${container} ${isBot ? botStyle : userStyle}`} style={{ boxShadow: isBot ? `0 18px 40px -20px ${hexToRgba(accent, 0.28)}` : "none" }}>
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center space-x-2 opacity-70">
            <button onClick={() => onCopy && onCopy()} className="p-1 rounded hover:bg-white/6">
              <Copy size={14} />
            </button>
            <div className="text-[10px]">{msg.ts ? new Date(msg.ts).toLocaleTimeString() : ""}</div>
          </div>
        </div>
