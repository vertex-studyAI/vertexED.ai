    ...
    <>
      ...
      <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
        <div className="orb-field">
          <span className="orb o1 animate-drift-slow" />
          <span className="orb o2 animate-drift-medium" />
          <span className="orb o3 animate-drift-fast" />
          <span className="orb o4 animate-drift-slow" />
          <span className="orb o5 animate-drift-medium" />
          <span className="orb o6 animate-drift-fast" />
        </div>
        <div className="absolute inset-0 texture-noise opacity-[0.16]" />
        <div className="absolute inset-x-0 top-0 h-64 bg-gradient-to-b from-white/6 via-transparent to-transparent" />
      </div>
      ...
      <section className="fade-up px-6 py-10">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-8">
          <div className="flex-1">
            ...
            <div className="mt-6 grid max-w-2xl grid-cols-2 gap-3 text-xs text-white/72 sm:grid-cols-4">
              <div className="rounded-2xl border border-white/10 bg-white/6 px-3 py-3 backdrop-blur-xl">
                Immersive UI
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/6 px-3 py-3 backdrop-blur-xl">
                Secure by default
      ...
      <section className="px-6 pb-12">
        <div className="max-w-7xl mx-auto">
          <div ref={containerRef} className="glass-panel p-6 md:p-8 lg:p-10 relative">
              ...
              {tiles.map((t) => (
                <Link to={t.to} key={t.title} className="group block tile-wrapper" aria-label={`${t.title} — ${t.info}`}>
                  <div className="tile-shadow h-full rounded-xl transition-all duration-400" style={{ boxShadow: "0 12px 40px rgba(0,0,0,0.25)" }}>
                    <div
                      className="tile h-56 md:h-64 w-full rounded-[28px]"
                      role="button"
                      tabIndex={0}
                      aria-hidden={false}
                    ...
                    >
                      <NeumorphicCard
                        className="h-full p-6 glass-tile-translucent flex flex-col justify-between transition-all duration-400 group-hover:border-white/20 group-hover:bg-white/10"
                      >
                        <div>
                          <div className="flex items-center gap-4 mb-2">
                            <div className="flex h-11 w-11 items-center justify-center rounded-full border border-white/15 bg-white/10 text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.15),0_14px_32px_rgba(0,0,0,0.18)] backdrop-blur-md" aria-hidden>
                              {t.icon}
                            </div>

                            <div>
                              <h3 className="text-2xl font-semibold text-white">{t.title}</h3>
                              <p className="mt-1 text-xs text-white/60">{t.info}</p>
                            </div>
                          </div>
                        ...
                        <div className="flex items-center justify-between mt-3">
                          <span className="text-xs text-white/45">Quick open</span>
                          <span className="text-sm font-medium text-white/75 transition-colors group-hover:text-white">Open →</span>
                        </div>
                      </NeumorphicCard>
                    </div>
                  </div>
      ...
      {/* Footer CTA */}
      <section className="fade-up px-6 pb-12">
        <div className="mx-auto max-w-6xl rounded-[32px] border border-white/10 bg-white/5 px-6 py-8 text-center shadow-[0_28px_90px_rgba(0,0,0,0.22)] backdrop-blur-xl md:px-10">
          <h3 className="text-2xl font-semibold text-white md:text-3xl">Ready to get back into flow?</h3>
          <p className="mx-auto mt-3 max-w-2xl text-white/68">
            Use the dashboard to jump straight into focused work — every tool in one calm, glassy workspace.
          </p>

          <Link to="/study-zone" className="mt-6 inline-flex items-center justify-center rounded-full border border-white/15 bg-white/10 px-8 py-3 font-semibold text-white shadow-[0_16px_45px_rgba(0,0,0,0.22)] transition hover:-translate-y-0.5 hover:bg-white/15">
            Start a session
          </Link>
        </div>
      </section>
