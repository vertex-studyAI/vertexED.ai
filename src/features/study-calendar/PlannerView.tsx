...
const PlannerView: React.FC = () => {
  ...
  const saveEditedTask = () => {
    ...
    try {
      ...
      setEditOpen(false);
    } catch (error) { setEditError(plannerPlanError(error, "edit-task")); }
  };
  ...
  const addTask = async (event: React.FormEvent) => {
    ...
    if (!useAi) {
      try {
        ...
        setAiOpen(false);
      } catch (error) { setAiError(plannerPlanError(error, "add-task")); }
      return;
    }
    ...
    try {
      ...
      if (revision === requestRevision.current) setAiError(plannerPlanError(error, "suggest-task"));
    } finally {
