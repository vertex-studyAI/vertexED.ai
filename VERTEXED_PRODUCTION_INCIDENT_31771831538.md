...
So the current incident is **not a broad site outage**. It is an **immutable deployment identity / health-contract failure**.

## Source contract

Current `api/_handlers/health.js` resolves the served revision from:

1. `VERCEL_GIT_COMMIT_SHA`;
2. `GITHUB_SHA`;
3. generated `BUILD_REVISION`.

When present, the value is emitted in both JSON field `revision` and header `X-VertexED-Revision`.

Current `vercel.json` build command is:

`VERTEXED_REQUIRE_BUILD_REVISION=1 ROLLUP_SKIP_NODEJS_NATIVE=true npm run build`

and `prebuild` runs `scripts/generate-build-revision.mjs`. With `VERTEXED_REQUIRE_BUILD_REVISION=1`, a deploy-relevant build is intended to fail closed rather than create an unverifiable artifact.

The immutable revision-stamp health contract was merged before the expected revision, in commit `cd50ee11c51a2f5ee337063a895d96053c2877ab` on 2026-08-12.
...
This evidence is enough to narrow the blocker, but it is **not** enough to claim the precise Vercel build error. Authenticated Vercel logs are outside the current connected surface.

Do not guess whether the failed build was caused by dependency installation, project settings, environment variables, build-revision generation, quota, or another Vercel error.
