...
Current `vercel.json` requires:

`VERTEXED_REQUIRE_BUILD_REVISION=1 ROLLUP_SKIP_NODEJS_NATIVE=true npm run build`

and the `prebuild` lifecycle runs `scripts/generate-build-revision.mjs`. The immutable revision-stamp health contract was merged before the expected deploy revision, in commit `cd50ee11c51a2f5ee337063a895d96053c2877ab`.
...
That evidence is sufficient to narrow the blocker, but not to claim the precise Vercel build error. Authenticated Vercel build logs are outside the current connected surface.

Do not guess whether the failure is dependency installation, project settings, environment variables, build-revision generation, quota, or another Vercel error.
