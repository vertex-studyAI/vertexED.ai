...
## Immersive system extension, 13 September 2026

The current landing keeps the established cobalt, white, ink, logo, study vocabulary, and revision trace. Liquid glass is reserved for navigation and bounded interactive surfaces. Blue atmosphere, soft line texture, and glass boundaries now extend across non-landing routes through the shared layout, without replacing each tool's reading surface or interaction model.

The playable Revision Stack is a contained study-break object, not a generic decorative background and not evidence of learner progress. It is allowed as an explicit interaction inside its own section. Do not spread falling blocks or a grid behind reading, forms, authentication, or study workspaces. Testimonials require verified people and permission; until then, use labelled research prompts rather than plausible names or quotations.

### Revision atelier refinement

The cohesive aesthetic language is a revision atelier: precise Inter typography, restrained Georgia annotations, cobalt rules, circular learning traces, white reading paper, and stable night-blue instrument surfaces. The original 8px control, 12px paper, and 24px glass-shell radius hierarchy applies across the landing. Shadows use one low, blue-grey elevation recipe. Inverted sections keep the same night-blue palette in light and dark modes instead of reversing independently.

A slim page trace and active glass island connect the chapters. Both are navigation context, not learner progress. Major panels share the same border, radius, highlight, and material rules. Subject accent colors remain orientation aids inside that system. On small screens Apex uses a smaller landing-only footprint so it can sit beside the section island without covering primary actions.
# September 13: revision workspace refinement

The landing hero uses one large illustrative revision card, with a random initial subject and explicit subject/stage controls. Keep cobalt, white, navy, editorial serif accents and the shared revision trace. The block game is a quieter reading-section companion, not a primary call to action. Testimonials remain explicitly fictional design previews until verified quotes replace them. Founder credits are equal: Ritayush, Ryan, Pratyush and Aadi.
# Apex command workspace and navigation, September 13
