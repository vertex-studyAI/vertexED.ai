...
Apex sits in the lower-right corner at 96px on desktop and 72px on mobile. A labelled button opens an opaque study-shortcut sheet. Plan, Focus and Practise lead to existing tools. Asking the AI tutor opens the existing Apex panel when available; do not add a second chat system. Only one tutor/companion launcher is shown at once. Keep him out of authentication, onboarding, admin and legal screens. Simple Mode suppresses him. The footer and shortcut sheet expose a hide control, stored only as a device preference. Three small Hop, Wiggle and Spin controls play finite reactions without changing learner data. No automated nudges, streak pressure, invented progress or learner monitoring.
