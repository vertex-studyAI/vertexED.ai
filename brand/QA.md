...
# Login, concept lens and automatic blocks, 13 September 2026

Later navbar/INC pass: ten focused browser cases pass on the final build. Captures cover 1440px, 1024px and 390px, light/dark navigation, discoverable default-on lens, expandable practice, draft reset, keyboard focus return and bounded image/neural effects. Correction pass restored tablet side margins, changed dark dialogs to navy and relaxed headline spacing. Fifteen auth regression cases, typecheck, scoped lint and copy lint pass; 285 copy files have zero findings. Final asset budgets pass (240040 bytes initial gzip JavaScript and 42205 bytes initial gzip CSS). The live Supabase settings endpoint confirms Google/email enabled, but real account login remains blocked by absent local server credentials; the live schema also lacks the newer school/profile fields. No production changes or live account mutations were made. See `docs/INC_UI_AUDIT.md` for the full distinction between UI adaptation and integration readiness.

- Fixed missing preview API middleware. An anonymous request to the actual local `/api/waitlist-status` endpoint now returns JSON HTTP 401 instead of SPA HTML. No access bypass or account mutation.
- Email login no longer applies the signup password-length policy. Browser verification intercepts authentication with a fixture error; it does not prove a live account login or Google-provider configuration.
- Six browser cases pass after the correction pass, covering 1440px, 1024px and 390px, light/dark captures, lens keyboard exploration, original cooling-rate practice, manual block placement, reduced motion, automatic movement, offscreen pause and explicit pause.
- Inspected the captures and reduced the block-section texture, narrowed the mobile board and enlarged graph labels. Fifteen authentication regressions pass. Copy lint scans 283 files with zero findings. Final built assets pass the existing size budgets.
- Hosted OAuth callback allowlisting and a real approved-account sign-in remain unverified. Local callback configuration does not update the hosted Supabase project. No deployment was made.
