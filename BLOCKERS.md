...
## P1 — Darcy v2 executable pre-training freeze
`DARCY-FREEZE-001 / darcy-operator-ood-v2` is already frozen before any v2 outcome. The remaining gate is `DARCY-EXEC-FREEZE-002`: commit the exact generator/source revision, split/data manifests and hashes, reference-solver tests, FNO/DeepONet implementations, library versions, parameter budgets, optimizer/tuning grid, seeds, hardware identity and compute/time cap required by the protocol. No v2 training, ID-test or OOD-test outcome is authorized until those inputs are immutable and checked. Material post-outcome changes require a new protocol version.

## P1 — FinanceMeta production authorization/security verification
The canonical source repository `build-the-future-11/finance4all-global-reach` is readable through the connected GitHub surface at `fbdd503223edc5b1780509720391083f485a4a85`, but a fresh isolated branch-creation attempt returned `403 Resource not accessible by integration`; write access is therefore **not** available through this connector. The connected Supabase surface currently exposes only VertexED project `xwlrzgfuhfbckgvcmyoq`, not FinanceMeta. Source inspection confirms an authorization boundary requiring hardening: `profiles.role` is consulted by privileged helper functions while authenticated users can update/insert their own profile under policies that do not preserve role, and `SECURITY DEFINER` helpers in the initial migration do not pin `search_path`.

Smallest unblock: grant GitHub contents/branch/PR write access to the target and connect the FinanceMeta production Supabase project. Only then prepare source hardening on an isolated exact-base branch, generate the migration through the repository's Supabase migration workflow, add denial-path regression tests, and verify member role-escalation denial/RLS/helper execution boundaries before any production application. Repository read visibility is not production authorization.
...
Canonical writable target/runtime/Supabase surfaces remain unavailable from this execution surface. The connected Supabase surface currently exposes only VertexED, not The Bu1LD. Production authorization, RLS/role boundaries, deployment identity and authenticated seven-role/golden-journey evidence therefore remain externally blocked. Do not substitute control-repo activity for production evidence.
