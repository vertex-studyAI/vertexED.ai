...
**As of:** 2026-08-14 after Darcy v2 protocol freeze  
...
| 4 | Darcy | frozen v2 asks whether resistance-preserving coarse compression survives arithmetic/log-mean, PCA-ridge, eligible FNO/DeepONet and fixed ID/OOD regimes | scientific-ML researcher; review the frozen protocol now, and independently reproduce only after the canonical pre-execution implementation/environment/split/model-budget/dataset-hash package is complete | **PROTOCOL READY / EXECUTION PACKAGE NOT READY:** `DARCY-FREEZE-001 / darcy-operator-ood-v2`; no v2 outcome exists | after a frozen eligible run, H1 plus H2 or a strong pre-specified negative/boundary result may justify a package | preserve a learned-baseline win or failed H1/H2 as the result; do not rescue by changing generator/baselines/gates |
...
The v2 protocol may be externally reviewed now, but execution/reproduction must wait until its required implementation/environment, deterministic generator + split manifest, model revisions/budgets, dataset hashes and compute identity are committed and independently checked. Protocol review is not experiment validation, and protocol readiness is not a positive scientific result.
